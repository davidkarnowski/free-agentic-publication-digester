...
# Collector liveness (after the continuous-ingestion workstream)
...
| After a collector change | `collect.py --once --no-llm` + collector_state read | cycle integrity without token spend |
