...
| Host fail2ban | Cohabitant-owned config; FAPD adds exactly one jail, `fapd-mcp` (reads `/opt/fapd/logs/mcp-access.log`, bans in `DOCKER-USER`), **installed 2026-09-14** from the operator's private host tree (security configuration is not published) |
...
| `fapd-mcp` fail2ban jail installed, `f2b-fapd-mcp` chain present in `DOCKER-USER`, log's first field a client address | 2026-09-14 (C-6 done: jail running, `f2b-fapd-mcp` chain present in `DOCKER-USER` from jail start, `fail2ban-regex` matches the rejection lines and nothing else; the log's first field is a public client address. Jail, filter, logrotate snippet and installer live in the operator's private host tree — not in this repository) |
