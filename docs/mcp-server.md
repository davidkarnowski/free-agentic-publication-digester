...
A 2026-07-28 request carries the version in `params._meta` (with
`clientInfo` and `clientCapabilities`) and in the `MCP-Protocol-Version`
and `Mcp-Method` headers. A request without them is a legacy-era
request, under which `server/discover` does not exist: the answer is
200 / `-32601`, as the specification requires, and since 1.1.0 the
message names the request form the client needs (an outside field
report on 2026-09-14 read the bare "Method not found" as "the
advertised method is not implemented").
...
`^[a-z0-9][a-z0-9-]{0,62}$`; `collection` `^[A-Z][A-Z0-9_]{1,15}$` and,
since 1.1.0, an enum of the nine collection codes the pipeline can
file (a value outside it is refused by name with the list); `agency`
(1.1.0) any printable characters up to 120, compared whole and
case-folded against `items[].agency` — never a substring, never a
path — with the result's `facets.tags` naming what is present. The
...
- 2026-09-14 (evening), **surface 1.1.0** — Wave A of
  [plan-2026-09-14-mcp-field-report.md](ops/plan-2026-09-14-mcp-field-report.md),
  from an outside agent's end-to-end field report: `get_live_day` and
  `get_day_listing` take `agency` (whole name, case ignored) and return
  the file's `facets`, so "what did agency X publish this week" is one
  call per day instead of paging every item; `collection` is an enum of
  the nine codes and an unknown value is refused by name with the list;
  a modern-only method sent in the legacy form gets a message naming
  the 2026-07-28 request form; `list_sources` says it is the summary.
