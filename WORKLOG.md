...
## 2026-07-31 — Source health, and the hardest page on the site to keep opinion-agnostic

The source guide listed 127 sources and said nothing about what any of
them actually gave us. The operator wanted that fixed in both
directions at once: we should be able to see where ingestion is broken,
and a reader should be able to see how well an agency's channel serves
citizens. Both are answered by the same mechanical facts, which is why
one page can carry both audiences.

The trap was obvious before a line was written. A "health" page is the
easiest place in this project to start grading agencies, and grading
agencies is precisely what GUIDE §2 forbids. The rule adopted, and
written into `health.py`'s docstring so it survives the next edit: every
label describes **our ingestion of a source**, never the publisher.
"None of 40 requests returned content" is something we observed. "This
agency is unreliable" is an opinion, is not derivable from anything in
the databases, and does not go on the site. Where a number could be read
as blame the page states the mechanical reason in place — a 5xx is a
server declining to return content, which may be load, maintenance,
on-demand generation, or a limit the publisher sets, and we cannot tell
which from outside, so we say that rather than guess. A test scans the
label vocabulary for verdict words and fails if one appears.

**Five labels, five constants, and nothing a reader cannot check.**
`delivering`, `quiet`, `degraded`, `no response`, `no data`. The whole
classification lives in named module constants — a 14-day window, quiet
past 7 days, degraded at 10% of requests returning no content or 3
consecutive failed collector cycles, and a 5-request floor below which a
percentage is noise rather than a signal. The page prints all of them,
and every card prints the numbers its own label came from, so a reader
can redo the arithmetic. The label glossary is generated by
interpolating the live constants into the sentences rather than
restating them, because a hand-copied threshold is a threshold that
drifts.

The 14 days is ten business days plus two weekends: a source that
publishes only on weekdays gets measured over whole cycles of its own
rhythm. The 7-day quiet mark clears a Friday-to-Tuesday holiday gap, so
an ordinary long weekend never registers as an interruption. The 10%
error rate was the one real judgement call. Three days of logs showed
882 of 4,868 govinfo requests coming back 503 — 18% — and GUIDE §3
already documents that govinfo answers 503 while generating ZIP and MODS
on demand, so those are partly expected. At 20% they would have sat just
under the line and the page would have said nothing about the single
largest source of failed requests we have. 10% says it plainly and lets
the mechanical reason do the explaining.

**Attribution is by host, because that is the unit the evidence
supports.** The fetch log records a URL, not a source id, so per-source
request health has to be derived from the host. That is not free:
all five govinfo collections read from `api.govinfo.gov` and therefore
show identical request figures, so each card says so — "this host serves
5 registered sources, so these figures are host-wide". Better a
disclosed approximation than a per-source number we cannot actually
observe. The govinfo cards also attribute to `api.govinfo.gov` rather
than the `www.govinfo.gov` collection page the registry links, because
the API host is the one we actually call.

**Email sources have no fetch table, and say why.** They are delivered
to us; we make no requests. Rendering an empty table of zeroes would
have been indistinguishable from a host answering nothing, so the email
path shows a sentence instead and takes its health from delivery
recency alone. The same instinct drove the missing-fetch-log case: when
the request log is absent but the pipeline database is not, request
statistics are omitted rather than read as zero, because an absent
measurement and a total outage must never look alike.

**Content length turned out to be the most revealing number on the
page.** Measured on the live databases before this work began (this
worktree has no `data/`, so the figures are the operator's, not this
session's): `justice-newsroom` averages 324 characters an item and
`usattorneys-email` 310 — those are teasers — while `cisa-advisories`
averages 28,909 and the Federal Register 16,486. A source emitting
310-character stubs is giving a reader far less than one emitting full
text, and that is a fact about the channel, not a criticism of anyone.
The registry already records the mode (`feed-only`, `email-teaser`,
`full`), so the card shows the mode beside the number and explains what
it means.

Health is computed for `active` sources only. Planned, unavailable, and
excluded entries are not ingested by definition, and labelling them "no
data" would have read as an interruption rather than as a registry
status; each of those cards carries one line saying so, because silent
absence is the failure mode this project guards against hardest.

**Accessibility, since this page is where colour-coding is most
tempting.** No state is signalled by colour alone: every indicator
carries its word, a glyph, and its colour, and a visually hidden prefix
supplies the noun so "quiet" out of context still says what is quiet.
Contrast was computed over the composited chip tint on both `--card` and
`--stripe` in both themes; the worst number is 4.93:1. Quiet and no-data
deliberately keep the plain muted chip and differ only by border style,
because neither is an observation of trouble and neither should look
like one. The page's one table — the label key — goes through the
existing `_accessible_tables` helper rather than a hand-rolled wrapper,
so it inherits the focusable labelled scroll region and the `scope`
attributes that A11Y-03 already settled. No script was added; the whole
thing is server-rendered.

**The machine surface is `sources.json`, and it is deliberately not part
of `digests.json` or the feed.** Those two enumerate the official
record. Our operational statistics are not that, and an agent polling
for what the government published must never receive them as though they
were. The file carries the thresholds alongside the labels so an agent
can recompute any classification from the numbers in the same document,
plus an explicit scope statement saying these describe our ingestion and
not any agency. `llms.txt` says the same in its entry.

The graceful-degradation contract was the constraint that shaped the
module's shape more than any other: `build_site()` has to work with no
`data/` at all, which is every fresh clone and every CI run. Missing
databases return `available: False` with the reason named, the page says
"not available in this build" instead of showing zeroes, and the
existing degradation test was extended rather than replaced. The
database paths are injected through `build_site`, so the test suite
builds its own throwaway SQLite files — 44 new tests, 420 total, ruff
clean. Two new seams recorded in the standards table.

One thing deliberately not built: unavailable sources show no request
history, even though their refusals are exactly the kind of
accountability data this project keeps. Those probes predate the
14-day window and did not all go through the fetch log, so showing "no
data" against them would have misrepresented a documented refusal as an
...
## 2026-08-24 — ten days the finalizer could not finish, and the floor it should have stood on

Came back after two weeks away from the log. The repo's last automated
evidence commit was 2026-08-14; the box had digests through 08-23. The
health check said everything was fine — `eod` row clean, push clean,
site current — because the 08-24 finalizer had succeeded and swept ten
days into one commit (`492ef05`). Only the docker log, which reaches
back three days, still had the failures: `EOD finalizer HALTED for
2026-08-21 after 3 failed attempts`, then the same for 08-22.

The chain, read from the ledger rather than the reports. On 08-14 at
23:08Z the `claude` CLI backend began answering "Your organization has
disabled Claude subscription access for Claude Code · Use an Anthropic
API key instead." Thirty-five zero-billed failures. On 08-15 production
moved to Gemini by a staged script — `LLM_BACKEND=gemini`, free tier —
and the ledger from then on shows 30–50 calls a day with 16–24 of them
429 `RESOURCE_EXHAUSTED`: the quota metric names itself in the body,
`generate_content_free_tier_requests, limit: 20`. The CLI era ran
~80 calls a day. The daily quota resets at midnight Pacific, 07:00Z;
the finalizer fires at 04:00Z, on a quota the daytime analyze worker
had already spent. `GeminiBackend.complete` makes exactly one HTTP
call (the 3-attempt backoff added in a4058cc was removed again in
3e88577), `LLMClient` retries once with no delay, `run_pipeline.py`
has no try/except around analyze, so one 429 was exit 1 — and the
ladder's spacing is the generic worker backoff, 10/20/40 minutes, so
all three attempts were spent by 06:12Z, before the reset. Eight
digests were regenerated by hand the following afternoon (the
"Generated at" stamps say 14:41, 13:18, 14:59, 17:50, 17:51, 16:55,
20:14, 12:37 UTC). None of those hand runs pushed anything, because
`run_pipeline.py` never calls the evidence commit — only the
supervisor's EOD path does — and the HALT message that told the
operator to run it says the next success clears the ladder, which a
manual run also does not.

Tonight's fix was the smallest one: `claude -p` inside the container
works again, and so does the project's own `CLIBackend` path, so the
box is back on `LLM_BACKEND=cli` (staged script, `.env` backed up,
container recreated 22:28Z, 29 workers cycling clean within a minute).
The Gemini key stays configured for a failover that does not exist yet.

The real fix is the operator's ruling, now GUIDE §6 rule 15: the
mechanical digest is the floor. Every day finalizes with or without
inference; the digest says only that no inference was available and
that its content is source-derived or mechanically constructed — no
causes, no vendor names, no timestamps; those go to the manifest and
the insight report. No prose backfill into a frozen day. Plan in
`docs/ops/plan-2026-08-24-eod-llm-resilience.md`, segmented by kind
(bug / feature / operations / housekeeping) because that is how the
branches are cut; the doc audit that went with it is
`docs/doc-audit-2026-08-24.md` — 32 items, the eighteen commits of
08-15..08-20 (Gemini, TTS, activity graph, static assets) having landed
without a WORKLOG line or a §14 decision between them. Work starts on
`arch/mechanical-digest`.

Also checked, and clean: the VPS itself. Two orderly reboots (08-20
with a kernel update, 08-23 without one), ufw on three ports, fail2ban
with three lifetime bans, nginx serving 1,503 two-hundreds and three
WordPress-probe 404s in 48 hours, cert good to October. govinfo's 22%
error rate is still the on-demand-ZIP 503 baseline — every USCOURTS URL
fails twice and then succeeds, nothing is stuck. One genuine stray:
