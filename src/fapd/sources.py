...
def render_doc(entries: list[dict]) -> str:
    ...
    lines = [
        ...
        "stays in sync.*",
        "",
        "*Read the Notes column as a **dated log**, not as current state. Each",
        "entry is appended when it is learned and never rewritten — a refusal,",
        "a failed convention guess, or a budget figure quoted at the time of a",
        "probe all stay as written, because a probe record that gets edited to",
        "match today stops being evidence. Where a note cites a request budget,",
        "check `src/fapd/config.py` for the figure in force now: the agency",
        "class was raised 500 -> 1,500 (2026-07-31) -> 3,000/day (2026-08-06),",
        "so older notes quote smaller caps than apply today.*",
        "",
        "## Coverage summary",
        "",
