...
7. **Token ledger, like the fetch log.** Every LLM call is logged (UTC
   ...
   *Provider record (added 2026-08-24).* Production ran the `claude` CLI
   backend from launch until 2026-08-14, when the vendor disabled
...
- **Rule 15 — the mechanical digest is the floor (added 2026-08-24,
  operator).** The pipeline finalizes every publication day with or
  without an inference provider. Model layers are additive. When no
  inference was available for a day, the digest states that fact in its
  own prose and nothing more; the cause is operational detail recorded
  in the day's provenance and operations report, not in the published
  digest. The Coverage Statement's arithmetic reconciles regardless.
  Items are listed with their citations whether or not they were
  summarized. A day finalized without model layers is frozen like any
  other day; prose is not backfilled into a frozen digest.
  *Incident this ratifies:* the `claude` CLI backend stopped
  authenticating on 2026-08-14 ("Your organization has disabled Claude
  subscription access for Claude Code"); production moved to Google
  Gemini on its free tier on 2026-08-15, whose request quota (~20/day
  against a digest that needs ~80–100) refused the end-of-day finalizer
  on eight of the next ten nights. One provider error was fatal to the
  whole run, the three-attempt ladder completed before the provider's
  quota reset, and eight digests were regenerated by hand the following
  afternoon while nothing reached the repository for ten days. The
  mechanism this rule requires: a provider that is disabled, refuses
  authentication, or exhausts its quota is a first-class state the
  client reports (`LLM_BACKEND=none` is a legitimate configuration, not
  an error); each model layer is attempted and its outcome recorded per
  day (`day_inference`); the render proceeds on stored rows and the §2
  gates decide as always; and the finalizer's retry ladder is reserved
  for validation failures and crashes, not for waiting on a vendor. The
  published disclosure is deliberately uninformative about causes: the
  digest is the public record of the publications, not of the
  operator's vendors, and a reader who needs the reason finds it in
  `provenance/runs/insight-<date>.md`. What this rule does not change:
  rule 5 (a stored summary stays stored), rule 13 (the analyze window),
